...
Object.defineProperty(exports, "__esModule", { value: true });
const webextension_polyfill_ts_1 = __webpack_require__(0);
const eventHandlers_1 = __webpack_require__(4);
const logic_1 = __webpack_require__(5);
// ========== MESSAGE HANDLERS ==========
/**
...
async function handleRuntimeMessage(msg, sender) {
    let tab = sender.tab;
    console.log('[tabId = %s][background_script:onMessage][%s] URL = %s', tab.id, Date.now(), tab.url);
    let url = new URL(tab.url);
    logic_1.processUrl(url);
}
;
webextension_polyfill_ts_1.browser.runtime.onMessage.addListener(handleRuntimeMessage);
webextension_polyfill_ts_1.browser.runtime.onInstalled.addListener(eventHandlers_1.handleRuntimeInstalledEvent);
 ...
 */
function handleRuntimeInstalledEvent(details) {
...
exports.handleRuntimeInstalledEvent = handleRuntimeInstalledEvent;
function handleExtensionInstalledEvent(details) {
    console.log('[background_script:handleExtensionInstalled] Details = %O', details);
}
function handleExtensionUpdatedEvent(details) {
    console.log('[background_script:handleExtensionUpdated] Details = %O', details);
}
function handleBrowserUpdatedEvent(details) {
    console.log('[background_script:handleBrowserUpdated] Details = %O', details);
}
function handleSharedModuleUpdatedEvent(details) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const notifications_1 = __webpack_require__(6);
const dataService_1 = __webpack_require__(2);
const DOMAINS = {};
async function processUrl(url) {
    // TODO: extract into function
    let site = url.hostname;
    let config = await dataService_1.loadServiceConfig(url);
    if (config === undefined) {
    ...
    notifications_1.popNotification(options, config);
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const webextension_polyfill_ts_1 = __webpack_require__(0);
const NOTIFICATION_ID_TO_CONTEXT = {};
async function popNotification(options, config) {
    let url = new URL(config.url);
    let notificationId = await webextension_polyfill_ts_1.browser.notifications.create({
        type: 'basic',
        // TODO: update to use the icon for this service from twofactorauth.org?
        iconUrl: webextension_polyfill_ts_1.browser.extension.getURL("icons/2fan_40px.png"),
        title: options.title,
...
webextension_polyfill_ts_1.browser.notifications.onClicked.addListener(function onNotificationClicked(notificationId) {
...
webextension_polyfill_ts_1.browser.notifications.onClosed.addListener(function onNotificationClosed(notificationId, byUser) {
    console.log('Notification with id %s closed byUser? %s', notificationId, byUser);
    getAndClearContext(notificationId);
});
function getAndClearContext(notificationId) {
